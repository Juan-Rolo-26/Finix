...
export default function TopGainersPage() {
    ...
    // Computed summary stats for the KPI cards
    const stats = useMemo(() => {
        if (!items.length) return null;
        const leader = items[0];
        const avgChange = items.reduce((acc, it) => acc + (it.changePercent || 0), 0) / items.length;
        const topVol = [...items].sort((a, b) => (b.volume || 0) - (a.volume || 0))[0];
        const maxAbsChange = Math.max(...items.map(it => Math.abs(it.changePercent || 0)), 1);
