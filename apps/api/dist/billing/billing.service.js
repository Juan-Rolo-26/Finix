...
let BillingService = class BillingService {
    ...
    async getCreatorSummary(userId) {
    ...
    async settlePendingBalance(userId) {
        await this.accessControlService.requirePro(userId);
    ...
    async requestPayout(userId, amount) {
        if (!Number.isFinite(amount) || amount <= 0) {
            throw new common_1.BadRequestException('El monto de payout es inválido.');
        }
        await this.accessControlService.requirePro(userId);
        ...
        if (!balance) {
            throw new common_1.BadRequestException('No existe balance para este creador.');
        }
        const available = Number(balance.availableBalance);
        if (amount > available) {
            throw new common_1.BadRequestException('No tienes balance disponible suficiente para este payout.');
        }
    ...
    async updateCommissionRate(rate) {
        if (!Number.isFinite(rate) || rate <= 0 || rate >= 1) {
            throw new common_1.BadRequestException('La comisión debe estar entre 0 y 1 (ej. 0.15).');
        }
