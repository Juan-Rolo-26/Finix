...
export class MetricsEngineService {
    enrich(response: FundamentalResponse): FundamentalResponse {
        ...
        if (!response.derived.sectorComparison) {
            response.derived.sectorComparison = {
                sector: null,
                percentile: score !== null ? clamp(Math.round(score), 1, 99) : null,
    ...
    private calculateSimplifiedDcf(response: FundamentalResponse): number | null {
        const fcf = response.metrics.freeCashFlow;
        const growthPct = response.metrics.revenueGrowthCagr;

        if (fcf === null || fcf <= 0) return null;

        const growth = growthPct !== null ? clamp(growthPct / 100, -0.1, 0.2) : 0.04;
    ...
    private calculateInternalScore(response: FundamentalResponse): number | null {
